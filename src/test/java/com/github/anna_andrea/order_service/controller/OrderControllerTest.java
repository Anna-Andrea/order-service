...
public class OrderControllerTest {
	...
	public void setup() {
		...
		validPlaceOrderQo = new PlaceOrderQo(Arrays.asList("37.7749", "-122.4194"),
	...
	public void testOrderList_ValidRequest() throws Exception {
		mockMvc.perform(get("/orders").param("page", "1").param("limit", "10")).andExpect(status().isOk())
				.andExpect(content().json(objectMapper.writeValueAsString(orderList)));

		// Test very large page and limit values
		mockMvc.perform(get("/orders").param("page", String.valueOf(Integer.MAX_VALUE)).param("limit", "1000"))
				.andExpect(status().isOk()).andExpect(content().json(objectMapper.writeValueAsString(orderList)));
	}

	@Test
	public void testOrderList_InvalidParams() throws Exception {
		// 1. Test Missing Parameter (MissingServletRequestParameterException)
		mockMvc.perform(get("/orders").param("limit", "1")).andExpect(status().isBadRequest()).andExpect(
				result -> assertTrue(result.getResolvedException() instanceof MissingServletRequestParameterException));

		mockMvc.perform(get("/orders").param("page", "1")).andExpect(status().isBadRequest()).andExpect(
				result -> assertTrue(result.getResolvedException() instanceof MissingServletRequestParameterException));

		// 2. Test Invalid Non-integer Values (NumberFormatException)
		mockMvc.perform(get("/orders").param("page", "abc").param("limit", "1")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(
						result.getResolvedException() instanceof MethodArgumentTypeMismatchException));

		mockMvc.perform(get("/orders").param("page", "12.5").param("limit", "1")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(
						result.getResolvedException() instanceof MethodArgumentTypeMismatchException));

		mockMvc.perform(get("/orders").param("page", "1").param("limit", "xyz")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(
						result.getResolvedException() instanceof MethodArgumentTypeMismatchException));

		mockMvc.perform(get("/orders").param("page", "1").param("limit", "11.4")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(
						result.getResolvedException() instanceof MethodArgumentTypeMismatchException));

		// 3. Test Invalid Integer Values (ConstraintViolationException)
		mockMvc.perform(get("/orders").param("page", "0").param("limit", "1")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(result.getResolvedException() instanceof ConstraintViolationException));

		mockMvc.perform(get("/orders").param("page", "1").param("limit", "0")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(result.getResolvedException() instanceof ConstraintViolationException));

		mockMvc.perform(get("/orders").param("page", "-1").param("limit", "1")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(result.getResolvedException() instanceof ConstraintViolationException));

		mockMvc.perform(get("/orders").param("page", "1").param("limit", "-10")).andExpect(status().isBadRequest())
				.andExpect(result -> assertTrue(result.getResolvedException() instanceof ConstraintViolationException));
	}
	...
	public void testPlaceOrder_ValidRequest() throws Exception {
		mockMvc.perform(post("/orders").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(validPlaceOrderQo))).andExpect(status().isOk());
	}
	...
	public void testPlaceOrder_InvalidParams() throws Exception {
		...
		mockMvc.perform(post("/orders").contentType(MediaType.APPLICATION_JSON).content(
				"{\"origin\": [\"34.0522\", \"-118.2437\", \"extra\"], \"destination\": [\"34.0522\", \"-118.2437\"]}"))
				.andExpect(status().isBadRequest()).andExpect(
						result -> assertTrue(result.getResolvedException() instanceof MethodArgumentNotValidException));
		...
		mockMvc.perform(post("/orders").contentType(MediaType.APPLICATION_JSON).content(
				"{\"origin\": [\"invalid_lat\", \"invalid_lng\"], \"destination\": [\"34.0522\", \"-118.2437\"]}"))
				.andExpect(status().isBadRequest()).andExpect(
						result -> assertTrue(result.getResolvedException() instanceof MethodArgumentNotValidException));
	...
	public void testTakeOrder_ValidRequest() throws Exception {
		mockMvc.perform(patch("/orders/{id}", "test-id").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(validTakeOrderQo))).andExpect(status().isOk());
	}

	@Test
	public void testTakeOrder_InvalidParams() throws Exception {
		// 1. Test @NotNull violation with status as null
		// (MethodArgumentNotValidException)
		mockMvc.perform(
				patch("/orders/{id}", "test-id").contentType(MediaType.APPLICATION_JSON).content("{\"status\":null}"))
				.andExpect(status().isBadRequest()).andExpect(
						result -> assertTrue(result.getResolvedException() instanceof MethodArgumentNotValidException));

		// 2. Test @ValidStatus violation with invalid status values
		// (MethodArgumentNotValidException)
		mockMvc.perform(patch("/orders/{id}", "test-id").contentType(MediaType.APPLICATION_JSON)
				.content("{\"status\":\"UNASSIGNED\"}")).andExpect(status().isBadRequest()).andExpect(
						result -> assertTrue(result.getResolvedException() instanceof MethodArgumentNotValidException));

		mockMvc.perform(patch("/orders/{id}", "test-id").contentType(MediaType.APPLICATION_JSON)
				.content("{\"status\":\"COMPLETED\"}")).andExpect(status().isBadRequest()).andExpect(
						result -> assertTrue(result.getResolvedException() instanceof MethodArgumentNotValidException));

		mockMvc.perform(patch("/orders/{id}", "test-id").contentType(MediaType.APPLICATION_JSON)
				.content("{\"status\":\"INVALID_STATUS\"}")).andExpect(status().isBadRequest()).andExpect(
						result -> assertTrue(result.getResolvedException() instanceof MethodArgumentNotValidException));
	}
